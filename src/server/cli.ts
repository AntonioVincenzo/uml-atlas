...
try {
  if (command === 'mcp') await startMcp(store);
    ...
    const initialized = await store.init(args.includes('--demo') ? exampleDocument : undefined);
    if (command === 'init') console.log(`Architecture initialized in ${root}`);
      ...
      const projectCatalog = new ProjectCatalog(); await projectCatalog.open(root, initialized.document.name);
      const server = createApp(store, uiDir, port, projectCatalog).listen(port, '127.0.0.1', () => console.log(`Atlas is ready at http://127.0.0.1:${port}\nWorkspace: ${root}`));
