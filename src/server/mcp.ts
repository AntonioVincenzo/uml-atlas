...
export function createMcp(store: Store) {
  ...
  server.registerTool('get_architecture', { description: 'Read the current architecture document, stable element IDs, and revision. Always read before proposing changes.', inputSchema: {}, annotations: { readOnlyHint: true } }, wrap(() => store.read()));
  server.registerTool('get_diagram', { description: 'Read one diagram as JSON and round-trip UML source, with the current revision.', inputSchema: { diagramId: z.string() }, annotations: { readOnlyHint: true } }, wrap(async ({ diagramId }) => { const current = await store.read(); const diagram = current.document.diagrams.find(d => d.id === diagramId); if (!diagram) throw new Error('Diagram not found'); return { revision: current.revision, diagram, uml: toUml(diagram) }; }));
  ...
  server.registerTool('get_history', { description: 'Read revision metadata, newest first.', inputSchema: {}, annotations: { readOnlyHint: true } }, wrap(() => store.history()));
  ...
  server.registerTool('read_code_region', { description: 'Read up to 200 lines from a workspace-relative code path. No execution. Rejects paths and symlinks outside this workspace.', inputSchema: { path: z.string(), startLine: z.number().int().positive(), endLine: z.number().int().positive().optional() }, annotations: { readOnlyHint: true } }, wrap(({ path, startLine, endLine }) => store.code(path, startLine, endLine)));
  server.registerResource('architecture', 'atlas://architecture', { description: 'Current architecture model and revision', mimeType: 'application/json' }, async () => ({ contents: [{ uri: 'atlas://architecture', mimeType: 'application/json', text: JSON.stringify(await store.read()) }] }));
