...
export function inspectDiagramGeometry(diagram: Diagram) {
  ...
  const connectors = diagram.edges.map(edge => {
    const source = byId.get(edge.source)!; const target = byId.get(edge.target)!; const sides = connectionSides(source.bounds, target.bounds); const start = port(source.bounds, sides.source); const end = port(target.bounds, sides.target);
    ...
    const labelPoint = { x: (start.x + end.x) / 2, y: (start.y + end.y) / 2 };
    return { id: edge.id, source: edge.source, target: edge.target, sourceSide: sides.source, targetSide: sides.target, start, end, segments, label: edge.label ? { text: edge.label, bounds: labelBounds(edge.label, labelPoint) } : null };
  });
