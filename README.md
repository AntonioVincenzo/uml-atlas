...
1. Add an element, choose its UML kind, and edit its properties in the inspector. Drag nodes to arrange them, or use **Tidy layout** to form deterministic left-to-right relationship layers. At forks and merges, tidy selects mutual preferred neighbors for straight, center-aligned tracks; cross-track relationships use top or bottom ports when space permits. It also reserves clearance between labels and endpoint nodes and reports remaining routing complaints. Fresh graphs authored in UML source receive this layout automatically. Hover over a node to reveal its top, right, bottom, and left connection handles, then drag between handles to create the selected relationship type. Click an edge to edit its endpoints, label, or multiplicities.
